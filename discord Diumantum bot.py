...
async def on_message(message):
    ...
    if message.content.startswith("hello"):
        embed1 = discord.Embed(title="Hello :wave:" , description=f"Hello {message.author.name}, How are you?", color=message.author.color)
    ...
    if message.content.startswith("help"):
        embed2 = discord.Embed(title="**Help menu**" , description=f"**Hello {message.author.name}, here are all comands (with prefix_):** \n hi, invite, server, redriot, amiuseful, spam, invite. \n **admin command:**\n ban, warn, kick", color=message.author.color)
